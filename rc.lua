...
beautiful.init("/home/volchara/.config/awesome/theme.lua")
...
globalkeys = gears.table.join(
    ...
    awful.key({ }, "#232", function () 
        awful.spawn.easy_async_with_shell("brightnessctl -d amdgpu_bl1 set 10%-", function()
            awful.spawn.easy_async_with_shell("brightnessctl get", function(out)
                brightness.value = tonumber(out)
            end)
        end)
    ...
    awful.key({ }, "#122", function () 
        awful.spawn.easy_async_with_shell("amixer set Master 10%-", function()
            awful.spawn.easy_async_with_shell('bash -c "amixer sget Master | grep -P -o -m 1 \'\\d{3}%|\\d{2}%|\\d{1}%\' | tr -d \\%"', function(out)
                volume.value = tonumber(out)
            end)
        end)
    end),
    ...
    awful.key({ modkey }, "s", function () 
        awful.util.spawn("flameshot gui") end,
                {description = "Launch screenshot making tool"}),
    ...
    awful.key({ modkey }, "c", function () 
        awful.util.spawn("flatpak run com.visualstudio.code") end,
...
awful.rules.rules = {
    ...
    -- Open application on corresponding tags
    ...
    properties = { screen = 1, tag = awful.screen.focused().tags[6], switchtotag = true } },
    { rule = { class = "Code" },
...
-- Remove gaps if only one client is open
